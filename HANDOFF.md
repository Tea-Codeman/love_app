...
从零构建「恋爱成长型社交小程序」v1 —— 以"关系成长"为核心驱动的微信小程序，让单身用户从陌生 → 好感累积 → 信任，最终促成真实伴侣关系。当前处于 **Implement 阶段**，M0–M4 全部完成；**M5 全收官（2026-08-30）**：M5.1–M5.4 落地部署、线上验收 PASS（SC5 rate=100 / 双计修复实证）、阈值回灌评估 #2 维持初值、**人工终审 21:40 签字通过**。**M6 全收官（2026-08-30 22:29 真机验收通过）：M6.1–M6.4 落地部署 + 真机验 PASS（管理员处置闭环 + SC5 真机有数 + 403/幂等补验通过）；冷启动准备/技术债留 M7。**M7 规划定稿 ✅（2026-08-30：范围 A 技术债清算 = 时区/streak 修复 + invite.js 核查处置 + 老对回填决策）+ **用户「放行 放行」签字批准动码（2026-08-30，紧接 M7 规划提交 c341247 之后）；**M7 全收官 ✅（人工终审 2026-08-30 23:32 用户签字通过）；M8 规划已启动（上线就绪 = 资质主体/隐私文案/内容安全切换/复合索引，待签字放行动码）**（🔴 M7.1 关键修正见「盲区防护」第 24 条：metrics-core.dayOfCST 已 +8，勿再补）**。**
...
**进度位置**：M0 已验收、M1 Checkpoint 通过、M2 已收尾、M3 已通过（含 BUG-1/BUG-2 真机复验 PASS）、M4 全部完成 ✅、**M5 全收官 ✅（2026-08-30：代码落地 + 线上验收 PASS + 阈值回灌评估 #2 + 人工终审 21:40 签字通过）**。**M6 规划定稿 ✅（2026-08-30：范围 A 收 M5 尾）**；**M6 全收官 ✅（2026-08-30 22:29 真机验收通过 + 人工终审）**。Checkpoint M4/M5/M6 均已终审放行。**M7 全收官 ✅（2026-08-30 23:32 人工终审用户签字通过：M7.1 时区/streak +8 修复部署 Active+云端代码核验、M7.2 invite.js 孤儿删+build 通过、M7.3 老对不回填决策已出）。M8 规划已启动（上线就绪 = 资质主体/隐私文案/内容安全切换/复合索引），待签字放行动码**。**
...
- ~~小程序账号主体/社交类目资质现状？内容安全云调用权限是否就绪？隐私政策文案进度？~~ ✅ **已纳入 M8 规划（M8.1–M8.4，2026-08-30 启动）：资质主体=M8.1 用户侧门禁、内容安全切换=M8.3、隐私文案=M8.2、复合索引=M8.4；待用户拍板 M8 范围后落码**。
...
- **M8 规划定稿**：`D:\Tencent\app\tasks\plan-m8.md`（上线就绪 = 资质主体/隐私文案/内容安全切换/复合索引，2026-08-30 启动规划，待签字放行）
...
1. **当前最重要的问题**：M0–M6 全收官（M5 终审 21:40 / M6 真机验收 22:29 均通过）；**M7 全收官 ✅（人工终审 2026-08-30 23:32 用户签字通过）**。**M8 已启动规划（上线就绪 = 资质主体/隐私文案/内容安全切换/复合索引），待用户签字放行动码**：M8.1 资质为门禁（用户侧微信后台先行）、M8.2 隐私文案与 M8.4 索引可并行、M8.3 内容安全强依赖 M8.1（个人主体调 msgSecCheck 报 48001）。
2. **从哪一步继续**：
