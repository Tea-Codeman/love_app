...
从零构建「恋爱成长型社交小程序」v1 —— 以"关系成长"为核心驱动的微信小程序，让单身用户从陌生 → 好感累积 → 信任，最终促成真实伴侣关系。当前处于 **Implement 阶段**，M0–M4 全部完成；**M5 全收官（2026-08-30）**：M5.1–M5.4 落地部署、线上验收 PASS（SC5 rate=100 / 双计修复实证）、阈值回灌评估 #2 维持初值、**人工终审 21:40 签字通过**。**M6 全收官（2026-08-30 22:29 真机验收通过）：M6.1–M6.4 落地部署 + 真机验 PASS（管理员处置闭环 + SC5 真机有数 + 403/幂等补验通过）；冷启动准备/技术债留 M7。**M7 规划定稿 ✅（2026-08-30：范围 A 技术债清算 = 时区/streak 修复 + invite.js 核查处置 + 老对回填决策）+ **用户「放行 放行」签字批准动码（2026-08-30，紧接 M7 规划提交 c341247 之后）；**M7 全收官 ✅（人工终审 2026-08-30 23:32 用户签字通过）；M8 已签字放行（2026-08-30 23:41）：M8.2 隐私正式文案 ✅ 已落地（build 通过）、M8.4 复合索引 ✅ 已建（reports/events ready）、M8.1 资质指引 ✅ 已出（用户侧微信后台待办·门禁 M8.3）、M8.3 内容安全 🔻 降级·维持 false（强依赖 M8.1 资质，延后 flip）**（🔴 M7.1 关键修正见「盲区防护」第 24 条：metrics-core.dayOfCST 已 +8，勿再补）**；**🔻 2026-08-31 M8.1 降级：资质申请中，v1 走受限邀请原型验证不公开上架**。** **2026-08-31 优化（O1/O4/O5 落地）：上架开关外置 server_config（免重部署）、隐私版本强制重同意、上架 Playbook 文档；O2 图像异步回调 / O6 分享入口留未来资质就绪后补。**
...
29. **M8 规划放行 + 部分落地（2026-08-30 23:41）**：用户签字「放行」启动 M8 落码。M8.2 重写 `src/pages/privacy/privacy.vue` 占位为正式合规文案（收集项/目的/存储/第三方共享/用户权利/未成年人/联系方式/更新机制，覆盖 SPEC §9 与 M8.2 字段清单），门禁链路不变；`npm run build:mp-weixin` DONE（EXIT=0，src/utils 完好）。M8.4 经 CloudBase MCP `writeNoSqlDatabaseStructure` 建复合索引：reports `status_createdAt`(status+createdAt)、events `eventName_day`(eventName+day) 与 `pairId_day`(pairId+day)，`listIndexes` 复核均 ready。M8.1 Agent 出用户侧资质指引清单（`tasks/m8-account-guide.md`：主体升级企业/个体 + 社交类目 + 内容安全权限，达标判定 + Agent 核验 msgSecCheck 非 48001）。M8.3 内容安全切换强依赖 M8.1 资质门禁，不擅自 flip `USE_WX_SECURITY`；图像 `mediaCheckAsync` 异步回调全仓无调用点记为缺口不实现。原子提交 + 推送；M8 待人工终审（M8.1 资质 + M8.3 真审）。

30. **M8.1 降级 · 走受限邀请原型（2026-08-31）**：用户「M8.1降级吧，我目前还在申请资质」→ 决策将 M8.1 资质门禁降级延后：v1 维持个人账号「受限邀请原型验证」模式（不公开上架），M8.3 内容安全同步降级维持 `USE_WX_SECURITY=false`（本地关键词兜底），不擅自 flip（个人主体调 msgSecCheck 报 48001）。M8.2 隐私正式文案、M8.4 复合索引已落地不受影响。依据 HANDOFF 第 38 行既有「上线路径决策：个人账号先做原型验证，上架门槛延后」。下一步 = 收官/灰度上线（受限邀请原型）：终态部署核验 + 主链路冒烟 + 收官文档 + 用户受限邀请实测；资质就绪后补 M8.1/M8.3 再公开上架。

31. **优化 O1/O4/O5 落地（2026-08-31）**：用户决策「现在不能上架、未来可能上架」→ 优化使未来切换成本最低。(O1) safety 上架开关外置为 `server_config` 文档(launch)，带 60s TTL 缓存，缺失回落 false；已建集合+文档(useWxSecurity:false)、重部署 safety 至 Active（CodeInfo 落地新代码）。未来上架在控制台翻一处即生效、免重部署。(O4) 隐私页加 `PRIVACY_VERSION=1.0.0`，storage 同意标记改版本号，App.vue 入口比对版本、落后强制重弹门禁。(O5) 写 `tasks/launch-playbook.md` 未来上架切换清单（资质→翻开关→补 O2 图像异步→M8.3 文本验收→恢复 O6 分享→隐私备案→审核）。O2 图像异步回调/O6 分享入口因个人号不可测/产品决策留未来资质就绪后。前端 build DONE。
...
**进度位置**：M0 已验收、M1 Checkpoint 通过、M2 已收尾、M3 已通过（含 BUG-1/BUG-2 真机复验 PASS）、M4 全部完成 ✅、**M5 全收官 ✅（2026-08-30：代码落地 + 线上验收 PASS + 阈值回灌评估 #2 + 人工终审 21:40 签字通过）**。**M6 规划定稿 ✅（2026-08-30：范围 A 收 M5 尾）**；**M6 全收官 ✅（2026-08-30 22:29 真机验收通过 + 人工终审）**。Checkpoint M4/M5/M6 均已终审放行。**M7 全收官 ✅（2026-08-30 23:32 人工终审用户签字通过：M7.1 时区/streak +8 修复部署 Active+云端代码核验、M7.2 invite.js 孤儿删+build 通过、M7.3 老对不回填决策已出）。M8 已签字放行（2026-08-30 23:41）：M8.2 隐私文案 ✅ 已落地、M8.4 复合索引 ✅ 已建、M8.1 资质指引 ✅ 已出（🔻 降级·用户申请中，延后）、M8.3 🔻 降级·维持 false（强依赖 M8.1 资质，延后 flip）**；**🔻 2026-08-31 M8.1 降级：v1 走受限邀请原型验证不公开上架**。**；收官/灰度上线（受限邀请原型）规划已起草（`tasks/plan-launch.md`），LA.1–LA.2 核验通过、LA.3 收官文档 `tasks/launch-readiness.md` 已出、LA.4 交接更新中。 **2026-08-31 优化（O1/O4/O5 落地）：safety 上架开关外置 server_config（已部署 Active，未来翻开关免重部署）、隐私版本+强制重同意、上架 Playbook `tasks/launch-playbook.md`；O2/O6 留未来。**
...
1. **当前最重要的问题**：M0–M6 全收官（M5 终审 21:40 / M6 真机验收 22:29 均通过）；**M7 全收官 ✅（人工终审 2026-08-30 23:32 用户签字通过）**。**M8 已签字放行（2026-08-30 23:41）并部分落地**：M8.2 隐私正式文案 ✅ 已落地（build 通过）、M8.4 reports/events 复合索引 ✅ 已建（ready）、M8.1 资质指引 ✅ 已出（`tasks/m8-account-guide.md`，用户侧微信后台待办）、M8.3 内容安全 🔻 降级·维持 false（随 M8.1 资质降级同步延后，个人主体调 msgSecCheck 报 48001，不擅自 flip）。**🔻 2026-08-31 M8.1 降级：v1 走受限邀请原型验证不公开上架，M8.3 待资质就绪再 flip。** 优化已落地：O1 开关外置 server_config（已部署）、O4 隐私版本重同意、O5 上架 Playbook（`tasks/launch-playbook.md`）；O2 图像异步回调/O6 分享入口留未来资质就绪后补。
2. **从哪一步继续**：
   ...
   - 若用户说"做收官/灰度上线/受限邀请" → 按 `tasks/plan-launch.md` LA.1(终态部署核验：7 函数 Active + build DONE + safety 线上 USE_WX_SECURITY=false 实证) → LA.2(主链路 + 受限邀请 gate 只读冒烟) → LA.3(`tasks/launch-readiness.md` 收官文档) → LA.4(HANDOFF/todo 交接更新)；M8.1 达标后再 flip M8.3。
...
- **下一步方向**：M0–M8 主体功能与上线就绪项（M8.2 隐私文案、M8.4 复合索引）已落地；**2026-08-31 M8.1 降级**（资质申请中，v1 走「受限邀请原型验证」不公开上架），M8.3 同步降级维持 `USE_WX_SECURITY=false`。**下一步 = 收官/灰度上线（受限邀请原型）**：终态部署核验 + 主链路冒烟 + 收官文档 + 用户受限邀请实测；资质就绪后补 M8.1/M8.3 再公开上架。阈值回灌待自然用户 ≥10 对（见 `threshold-calibration.md`）。
