...
- ✅ **M4.1 已部署并核验落地（2026-08-30 收尾）**：13 个事件插桩代码（checkpoint `5441779`）已全量部署 —— 6 个存量函数（auth/chat/game/growth/match/safety）经 `updateFunctionCode` 部署、`Status: Active` 且 CodeInfo 逐函数确认含 `require('./metrics-core')` 与各事件 `metrics.track(...)`（auth→mbti_completed/profile_completed；chat→message_sent/chat_unlocked/contact_unlocked/pair_stage_changed；game→game_join/game_done/pair_stage_changed；match→match_accept；safety→report_created；growth→pair_stage_changed），PII 过滤到位（report_created 仅 `targetType`、不报 `reason`）。**🔴 收尾中暴露并修复一个部署缺陷**：新建的 `metrics` 函数首次部署后虽 `Status: Active`，但 invoke 报 `Cannot find module 'wx-server-sdk'` —— 根因是 `cloudfunctions/metrics/` 漏了 `package.json`（其余存量函数自带），依赖未安装；已补 `cloudfunctions/metrics/package.json`（wx-server-sdk ~2.6.3）并重部署，CodeSize 由 6309B → 11110312B（~11MB，依赖已装），invoke 复测通过（无主事件触发 openid 缺失护栏，返回 `accepted:false`，符合 BUG-1 设计）。**真机联调（前端 `track.js`/`App.vue`/`match.vue` 的 `app_open`/`recommend_view`）已验证**：用户重跑 `npm run dev:mp-weixin` 并上传后，`app_open`×12 已实证（前端埋点生效）。`recommend_view` 计 0 为路径未覆盖（本轮走接受流程未进推荐页），非代码 bug。**✅ 真机验收已通过（2026-08-30 凌晨，查库 26 条）**：13 事件中 9 类已实证；`chat_unlocked`/`profile_completed`/`recommend_view` 计 0 根因与补测方式见 `tasks/verification-log.md` M4.1 章 + `tasks/m4.1-supplement-test.md`。
- ✅ **contact_unlocked 已幂等化（2026-08-30 部署）**：`chat/index.js` 的 `contact` 改为每 pair 首次解锁才报一次（查 events 中该 pairId 是否已有 `contact_unlocked` 判重），`chat` 函数已重部署 `Status: Active`。补测时该事件每个 pair 应仅 1 次。
- ✅ **M4.1 正式闭环（2026-08-30 05:0x 终验，查库 47 条）**：用户重建前端（fix `572be3d`）并跑完补测轮次 2/3 后，`profile_completed`×1（R188 填齐四项）、`recommend_view`×2（6LrPFY+R188 进匹配页，count=5）均已触发，**证明漏 import 的 bug 修复生效**；`chat_unlocked`×1（R188↔6LrPFY 全新 pair 首条消息）亦闭合。三断言全过：PII 零泄漏 / 白名单无越界 / day=CST(+8) / pairId 双 openid 排序。13 事件埋点全部实现、部署、真机验证，SC1–SC4 可观测管道打通。剩余 `contact_unlocked`/`report_created` 为「实现就绪、测试场景未覆盖」（`contact_unlocked` 本轮无 pair 达 S4 解锁门槛），不阻塞闭环。详见 `tasks/verification-log.md` M4.1 章「补测终验」小节。
