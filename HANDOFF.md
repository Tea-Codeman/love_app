...
19. **【拉黑闭环 + 撮合性能·2026-08-29】三件事**：
  ① **匹配页候选卡新增「拉黑」按钮**（带二次确认防误触），调既有 `safety.block`，成功后本地移除该候选——**补上社区隐藏后「黑名单只减不增」的缺口**。顺带修复空状态文案仍在引导用户去已被隐藏的社区（"去社区多聊聊吧"）。
  ② **`safety` 云函数新增 `listBlocks` / `unblock`**，新建 `pages/settings/settings.vue` 承载黑名单列表（解除按钮 + 二次确认），首页挂「设置 ›」入口。**安全要点**：`unblock` 必须 `where({ blockerId: OPENID, blockedId })` 限定——云函数以管理员权限运行，不过滤 `blockerId` 就能删掉**别人**的拉黑记录；`listBlocks` 用 `users.where({ openid: _.in(ids) })` **一次 join**（共 2 次查询，非 N+1），并显式 `.limit()` 避免默认截断在 100 条；被拉黑者已不存在于 `users` 时显示「已注销用户」（`missing` 标记）。
  **架构原则（已写入 safety 头部注释，务必遵守）**：拉黑是安全机制，**过滤只能由服务端执行**，前端只负责"显示"。若改成"前端持有列表 → 传给后端过滤"，客户端传空数组即可绕过，防骚扰能力**完全失效**（此外还有状态漂移、payload 无界增长、失去审计追溯三个问题）。非安全类的**体验偏好**过滤（如"本次会话已划过的候选人"）才适合由前端传参。
  ③ **消除 `recommend` 的 N+1**：原先对每个候选各查一次 `matches`（`Promise.all`，候选上限 100 即最多 100 次查询），改为 `aggregateDoneStats` 按批（`AGG_BATCH = 20`）一次查完再在内存分组，查询次数从 N 降到 `ceil(N/20)`；输出结构（`gameCount`/`gameTacit`/`score`）保持不变。
  **验证**：提取云函数实际代码 + mock db 跑 7 条用例（双向对局合并、`active` 对局排除、无关用户排除、空候选/未登录不发起查询、50 候选=3 次查询）全部 PASS；`safety`/`match` 两个云函数 `node --check` 通过；`build:mp-weixin` 编译通过。
  **待部署**：`safety` 与 `match` 均需重新上传部署（否则黑名单读不到、`unblock` 不存在）。
