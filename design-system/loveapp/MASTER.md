...
社区 · 关系 · **一起玩(居中突出 CTA)** · 我的 · 设置。
- 顺序即视觉顺序，一起玩恒为第 3 项居中（左侧 2 项、右侧 2 项，对称）。
-  removed "匹配" 侧Tab：它与中央"一起玩"都指向匹配破冰同一入口（`/pages/match/match`），
  保留 CTA 更聚焦，避免两个入口指向同一页造成的重复。
- "设置" 做成独立第 5 个导航键（指向 `/pages/settings/settings`）——用户反馈在「我的」页内
  找不到设置入口，故提升为导航级入口；「我的」页内仍保留齿轮作为冗余入口。
- 中央"一起玩" CTA 指向 `/pages/match/match`（匹配破冰/游戏发起页），而非 `/pages/game/game`
  （后者是具体对局房间，必须带 `gameId` 参数，不能作为导航入口）。
- 固定底部，白底 + 顶部发丝线 + `--shadow-lg`（上投）。
- 五项均带图标（community/relation/game/profile/settings，走 svgIcon 内联 SVG）。
