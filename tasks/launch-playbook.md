...
> 配套优化：O1 开关外置（`server_config.launch.useWxSecurity`）、O4 隐私版本+强制重同意、O2 已落地（imgSecCheck 同步内联，免回调）、O6 分享/邀请入口（待恢复）。

## 0. 一句话结论

公开上架 = 用户侧资质就绪 + 控制台翻 1 个开关（O1，免重部署）+ O2 已落地（同步 imgSecCheck 内联判定，免异步回调/HTTP函数/控制台配置）+ 恢复分享入口（O6）+ 隐私备案。文本真审无需改业务代码。

## 1. 前置依赖（用户侧 · 微信公众平台）

- [ ] **M8.1 资质**：账号升级企业/个体工商户主体（个人主体通常无法授社交类目）。
- [ ] **社交类目**：小程序类目选「社交-婚恋交友」并通过资质审核。
- [ ] **内容安全权限**：开通 `cloud.openapi.security.msgSecCheck` / `imgSecCheck`（通常需企业主体）。
...
### 步骤 2 · O2 图像内容安全已落地（同步 imgSecCheck，免回调；公开上架前无需额外动作）
- 当前 `checkImage` 在 `useWxSecurity=true` 时走 `wxCheckImage`（`cloud.openapi.security.imgSecCheck`），**同步内联返回判定**（`errCode===0` 通过、`87014` 违规），实现正确，无需异步回调。
- 前端上传图片前压缩至 ≤1MB，调用 `checkImage` 传 `media.base64`（不含 `data:` 前缀）；O2 已随 v1 基础设施就绪（全仓暂无图像上传调用点，启用即生效）。
- 翻步骤 1 开关后自动生效。社区有图片，翻开关即经真审，无需额外动作。
...
- [x] 图像内容安全已接（O2 · 同步 imgSecCheck 内联判定，免异步回调/HTTP函数/控制台配置），图片经真审（翻 O1 开关即生效）
