...
## M8.1 降级 · 收官/灰度上线（受限邀请原型）✅（2026-08-31）

### M8.1 降级决策（用户「M8.1降级吧，我目前还在申请资质」）
- **决策**：M8.1 资质门禁降级延后——v1 维持个人账号「受限邀请原型验证」模式（不公开上架）；M8.3 内容安全同步降级维持 `USE_WX_SECURITY=false`（本地关键词兜底），不擅自 flip（个人主体调 `msgSecCheck` 报 48001）。
- **依据**：HANDOFF 第 38 行既有「上线路径决策：个人账号先做原型验证，上架门槛延后」；SPEC T2 邀请裂变（分享卡片受限邀请）。
- **影响**：M8.2 隐私文案、M8.4 复合索引已落地不受影响；仅 M8.1/M8.3 延后至资质就绪。
...
- 主链路 社区→游戏破冰→关系升温→加微信导流 接线完整（M0–M7 真机验收 PASS）。
- 受限邀请 gate：`App.vue:39-40` 存 `pendingInviter`（分享卡片 `?inviter=`）→ `auth.js:11-19` 注册时上报 `inviteCode` 并清除；为唯一受限准入路径（不公开上架 + T2 分享驱动），无需新增代码。

### LA.3 收官文档 ✅
- `tasks/launch-readiness.md`：交付说明 + 受限邀请上线就绪清单 + 已知限制 + 用户手动步骤（资质申请→flip M8.3→上架）+ 阈值回灌条件 + 收官核验记录。

### LA.4 交接更新 ✅
- HANDOFF：进度位置/极简版补收官段 + 接手指南补 LA 指引 + 第 30 条里程碑（M8.1 降级）。
- `tasks/todo.md`：新增「收官/灰度上线」LA.1–LA.4 任务卡 + Checkpoint LA。
