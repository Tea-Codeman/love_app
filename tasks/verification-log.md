...
- **当前 blocks 状态**（用户重跑 block 测试）：1 条 `sJ8Fv8`→`6LrPFY`（即 `我不爱罗` 拉黑 `woailuo`），符合预期。
- **复测要点**：A 拉黑 B 后，B 驻留匹配大厅 ≤12s 内列表应自动移除 A；且 B 即使趁快照未刷新点了 A 的「一起玩」，也应收到「对方已不可见」提示且**不会**建局/产生邀请。
